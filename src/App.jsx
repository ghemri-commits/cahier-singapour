...
const CURRICULUM = [
  ...
  { grade: 3, units: [
    { id: 'g3u1', name: 'Multiplication & Division', icon: '③', lessons: [
      { id: 'g3u1l1', name: 'Tables de 3 et 4', type: 'arrayMultiply', tables: [3, 4], description: 'Multiplications en tableau' },
      { id: 'g3u1l2', name: 'Sens de la division', type: 'division', mode: 'share', description: 'Partager également' },
      { id: 'g3u1l3', name: 'Course : Multiplications', type: 'speedDrill', mode: 'multiplication', targetNumber: 5, description: 'Pratique de vitesse.' }
    ]}
  ]},
  { grade: 4, units: [
    { id: 'g4u1', name: 'Fractions', icon: '④', lessons: [
      { id: 'g4u1l2', name: 'Fractions équivalentes', type: 'fractionEq', description: 'Fractions égales' },
      { id: 'g4u1l3', name: 'Course : Soustractions', type: 'speedDrill', mode: 'subtraction', targetNumber: 50, description: 'Pratique de vitesse.' }
    ]}
  ]}
];
    ...
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(); }} className="flex flex-col items-center gap-4">
      <div className="relative">
        <input type="text" inputMode="numeric" pattern="[0-9]*" value={value}
          ...
          style={{ borderColor: accent }} placeholder="?" autoFocus />
      </div>
...
function StarMap({ count, accent }) {
  const stars = useMemo(() => {
    return Array.from({ length: Math.min(count, 50) }, (_, i) => ({
      id: i, x: (Math.sin(i * 135.4) * 40) + 50, y: (Math.cos(i * i * 2.5) * 40) + 50, size: (i % 3) + 2
    }));
  }, [count]);

  return (
    <div className="relative w-full h-32 bg-stone-900 rounded-2xl overflow-hidden mt-4 shadow-inner">
      <div className="absolute inset-0 opacity-20" style={{ backgroundImage: `radial-gradient(circle at 2px 2px, ${accent} 1px, transparent 0)`, backgroundSize: '15px 15px' }} />
      {stars.map(s => (
        <div key={s.id} className="absolute rounded-full animate-pulse"
             style={{ left: `${s.x}%`, top: `${s.y}%`, width: s.size, height: s.size, backgroundColor: accent, boxShadow: `0 0 ${s.size * 2}px ${accent}` }} />
      ))}
...
function SpeedDrillGame({ mode, targetNumber, accent, onComplete }) {
  ...
  const check = () => {
    if (parseInt(val) === problem.a) {
        setScore(s => s + 1); setVal(''); setProblem(generate());
    } else setVal('');
  };
  ...
  if (status === 'end') return (
    <div className="text-center p-10 bg-white rounded-3xl border-4 border-emerald-400 animate-bounce">
      ...
      <button onClick={() => onComplete({ score })} className="px-10 py-4 rounded-2xl bg-stone-900 text-white font-bold text-xl">Terminer</button>
    </div>
  );
...
function LessonView({ lesson, kid, onComplete, onExit }) {
  ...
  switch (lesson.type) {
    case 'numberBonds': return <NumberBondsLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'doubles': return <DoublesLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'makeTen': return <MakeTenLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'partWhole': return <PartWholeLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'numberLine': return <NumberLineLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'base10': return <Base10IntroLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'base10Add': return <Base10AddLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'base10Sub': return <Base10SubLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'compensation': return <CompensationLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'comparison': return <ComparisonLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'barModel': return <BarModelLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'arrayMultiply': return <ArrayMultiplyLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'fractionBar': return <FractionLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'fractionEq': return <FractionEqLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'problemSolving': return <ProblemSolvingLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'division': return <DivisionLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
    case 'geometry': return <GeometryLesson lesson={lesson} kid={kid} onComplete={onComplete} onExit={onExit} />;
          ...
          {config.kids.filter(k => k.enabled !== false && k.name?.trim()).map(kid => {
            ...
            return (
              <button key={kid.id} onClick={() => onPickKid(kid)}
                ...
                <StarMap count={stars[kid.id] || 0} accent={c.ink} />
              </button>
            );
          })}
        </div>
